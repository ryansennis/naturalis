"""
    Clases for primer vector analysis
"""
...
class PrimerVectorAnalyzer:
    """
    Analyzes trajectories using primer vector theory. 
    Implements primer vector propagation and analysis based on Lawden's necessary conditions 
    for optimal impulsive trajectories.
    """
    def __init__(
    ...
    def _dynamics(
        ...
        dAcc_dr = -self.mu / (r_mag**3) * (identity - 3 * outer_rr / (r_mag**2))
    ...
    def analyze_segment(
        ...
        for i, _ in enumerate(sol.t):
            ...
            p = (
                N_to @ inv(N_fo) @ (dv_f/norm(dv_f))
                + (M_to - N_to @ inv(N_fo) @ M_fo)
                @ (dv_o / norm(dv_o))
            )
    ...
    def analyze_trajectory(
        ...
        """
        primer_segments = []
        for i, _ in enumerate(trajectory.segments):
            segment = trajectory.segments[i]
            primer_segment = self.analyze_segment(
                segment,
                trajectory.burns[i],
                trajectory.burns[i + 1]
            )
    ...
    def midcourse_correction_matrix(
        ...
        """
        Calculate the matrix A that is used to calculate the 
        initial guess for the midpoint burn position.
        ...
        """
        phi0 = np.eye(6).flatten()
        y0_initial = np.concatenate(
            [phi0, segment.initial_state.position, segment.initial_state.velocity]
        )
